...
## 1. LM Studio Installation & Server Setup

1. **Install LM Studio:** Download and install [LM Studio](https://lmstudio.ai/) on your Windows host system.
2. **Download Models:**
...
## 3. Configuring Roo Code (Dual-Brain Architecture)
...
1. **Open Roo Code Settings:** In VSCode (running via WSL), open the Roo Code extension settings.
2. **Configure API Provider:** Set the API provider to `OpenAI Compatible`.
3. **Connect to Host (WSL Bridge):**
...
1. **Initialize MCP:** Navigate to the MCP configuration section in Roo Code (managed via the sidebar or configuration file).
2. **Tool Permissions:** Ensure Roo Code is granted explicit permissions to execute terminal commands (`bash`), manage version control (`git`), and perform file operations using the models.
...
---
Developed by the Auspicious-Days Contributor Circle.d)
