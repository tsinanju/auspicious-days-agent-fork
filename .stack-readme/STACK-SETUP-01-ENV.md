...
# (DRAFT)
# Step 1: Environment Setup (Arch-Headless-WSL & IDE)
...
* **IDE & Orchestrator:** VS Code with the Roo Code extension.

## 2. Installing WSL and Arch Linux via PowerShell
...
## 3. Base System & Package Management
   ...
   Upon your first login to the Arch WSL instance, you will be operating as the root user. You must initialize the package manager (pacman), update the keyring, and install essential baseline tools.
   > [!NOTE]
   > It is best practice to install `bun` after creating your non-root user, as the `bun` installation script targets the current user's home directory.
...
### 3.1 - Setting up a Non-Root User **(Recommended)**
   ...
   Running AI workflows and development tools as root is dangerous and can cause permission issues. Create a standard user account and switch to it before installing user-level tools like bun.
...
### 3.2 - Installing the Runtime (Bun)
   ...
   Now that you are operating as your standard user, install bun (our primary runtime and project manager). Note: It is best practice to install bun after creating your non-root user, as the installation script targets the current user's home directory.

   ```bash
   # Install bun to the user's home directory
   curl -fsSL [https://bun.sh/install](https://bun.sh/install) | bash

   eload your bash profile to add bun to your PATH
   source ~/.bashrc
   ```
...
4.1. **Configure Git (Conventional Commits):**
   ...
   OR
   ```bash
   # CD into your working directory ~/modname
   # verify you are at the root of your project
   pwd
   ...
   # Expected output:
      # file:/root/.gitconfig   user.your.email@example.com
      # file:/root/.gitconfig   user.name=Your Name
      # file:.git/config        user.your.email@example.com
      # file:.git/config        user.name=Your Name
   ```
...
Once VSCode is open and connected to WSL (verified by the green remote indicator in the bottom-left corner):
